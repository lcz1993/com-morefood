module.exports = class extends think.cmswing.admin {
  ...
  async addAction() {
    if (this.isPost) {
      ...
      if (res) {
        return this.success({name: '添加成功！', url: '/admin/slot_machines/index/?id=3'});
      } else {
        return this.fail('添加失败!');
      }
    } else {
      this.meta_title = '积分商品添加';
      const data = await this.model('activity').find(3);
      this.assign('data', data);
  ...
  async editAction() {
    if (this.isPost) {
      ...
      if (res) {
        return this.success({name: '修改成功！', url: '/admin/slot_machines/index/?id=3'});
      } else {
  ...
  async editActivityAction() {
    if (this.isPost) {
      ...
      if (res) {
        return this.success({name: '修改成功！', url: '/admin/slot_machines/index'});
      } else {
        return this.fail('修改失败！');
      }
    } else {
