...
module.exports = class extends think.cmswing.admin {
  ...
  constructor(ctx) {
    ...
    // 其他额外的操作
    this.db = this.model('setupapp');
    this.tactive = 'setupapp';
  }
  // 加载配置
  async loadsetup() {
    const fs = require('fs');
    const setup = await this.model('cmswing/setupapp').lists();
  ...
  async addAction() {
    if (this.isPost) {
      ...
      if (addres) {
        await this.cache('setupapp', null);
