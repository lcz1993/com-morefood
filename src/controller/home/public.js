...
module.exports = class extends think.Controller {
  ...
  async signinAction() {
    ...
    if (this.isAjax()) {
      ...
      const loginname = this.post('loginname');
      let password = this.post('password');
      password = encryptPassword(password);
      const res = await this.model('cmswing/manager').signin(loginname, password, this.ip, 1, 1);
      if (res.uid > 0) {
        // 记录用户登录行为
        // await this.model("cmswing/action").log("user_login","member",res.uid,res.uid,this.ip,this.ctx.url);
        // console.log(11111111111111);
        await this.session('managerInfo', res);
        // TODO 用户密钥
        // this.redirect('/admin/index');
        return this.success({name: '登陆成功!', url: '/home/index/index'});
      } else { // 登录失败
        let fail;
        switch (res) {
          case -1:
  ...
  async logoutAction() {
    ...
    if (is_login) {
      ...
      this.redirect('/home/public/signin');
    } else {
      this.redirect('/home/public/signin');
    }
  }
  ...
  async islogin() {
    const user = await this.session('managerInfo');
